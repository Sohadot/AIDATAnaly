...
## Sprint 13B-E — External Publication Log Activation
...
| Field | Value |
|-------|-------|
| **Status** | PASS (pending CI gate confirmation on push) |
| **Decision** | AGENT-READ-001 — `DECISION_AGENT_READABILITY_LAYER_1_0_RATIFICATION.md` |
| **Summary** | All 41 launch routes now carry exactly one governed JSON-LD block (schema.org: WebSite, WebPage, Organization, DefinedTermSet, DefinedTerm, WebApplication) generated by `scripts/inject-jsonld.py` from the governed registries. Root `llms.txt` added and required in `dist/`. `validate-pages.ps1` amended: JSON-LD required + validated per page; executable-script prohibition unchanged. Closes gap G4; serves criterion C2. |
| **Validators** | `validate-pages.ps1` (amended JSON-LD checks), `validate-dist.ps1` (+llms.txt), full quality gate |
| **Notes** | No route, sitemap, robots, scoring, or ontology changes. Structured data restricted to governed definitions only. |
